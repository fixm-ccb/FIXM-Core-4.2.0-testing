...
Note: FIXM is only concerned with information modelling. FIXM does not dictate how flight information is presented to human stakeholders. This is in line with the FF-ICE/R1 Implementation Guidance Manual Chapter 2 which explains that the processing of FF-ICE messages is expected to be automated, and that FF-ICE exchanges are not directly “human consumable”. A flight dispatcher or air traffic controller are not expected to be exposed to raw flight information encoded in FIXM. Rather, a user interface would be provided for such stakeholders, which presents the content of the flight information encoded in FIXM in an operationally meaningful manner.
